...
> The PHP versión for this project is stablished on php 8.3.
...
- **Production**: Use this project if your applications are meant to be deployed together or share infrastructure. Each app should have a valid reason to coexist in the same production stack.
- **Development**: You can use this environment to host multiple unrelated applications. It’s recommended to simulate a realistic architecture (monorepo, microservices, etc.). If running multiple instances of this environment locally, adjust the exposed ports in the `.env` fjile to avoid conflicts.

### 📦 Clone and initialize

💡 Tip: If using this project as a boilerplate, delete the .git folder after cloning to detach it from the original repository.
...
The project separates stack definitions in a modular way:
...
💡 In production, the htdocs folder should be empty, but is only a recommendation. Applications are expected to be exposed through this environment, not hosted inside it.
...
### 🌐 Clean port-based routing

Each application is bound to a dedicated Apache port, avoiding unsafe paths like /public.
...
And apply it in your docker-compose.yml:
...
- Avoids external DNS or host file edits.
